...
export default {
	...
	mounted() {
		...
		let zbl_userHabits = JSON.parse(sessionStorage.getItem('zbl_userHabits'))
		if(zbl_userHabits.dietHabits) {
			this.value = zbl_userHabits.dietHabits.split(',').filter(item => {
				return item.length
			})
		} else {
			this.value = []
		}
		this.value2 = zbl_userHabits.smokingHabits
		this.value3 = zbl_userHabits.drinkingHabits
		this.value4 = zbl_userHabits.exerciseHabits
	},
