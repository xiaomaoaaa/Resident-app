...
export default {
    ...
    methods: {
         Passwordvalid() {
                let params = [{"targetId":"708e8594-2972-421e-ae73-21228c100afc","targetType":"031","pageSize":20,"pageNo":1}]
        ...
        send(item){
        	//点击到跳转到已解约
        	this.$router.push("/jmzx_detail")
        	
        	//console.log(item.status)
        	console.log(item.applyId)
        	this.$store.dispatch('li_applyId',[item.applyId,item.provinceText,item.cityText,item.districtText,item.streetText,item.address,item.idOrNo,item.personName,item.phoneNo,item.teamName,item.mpiId,item.idType])
        },
