...
<div class="app" style="overflow-x:hidden;background-color:#F2F2F2;height:100vh;">
    ...
    <div class="header">
        <mt-header title="居住地址">
            <a slot="left" @click='$router.push("/mybaseinfo")'>
                <mt-button icon="back"></mt-button>
            </a>
            <a slot="right">
                <mt-button class="smallbtn" @click.native='saveAddress'>保存</mt-button>
            </a>
        </mt-header>
    </div>
...
export default {
    ...
    methods: {
        saveAddress() {
            if (document.querySelector('.selectCity').innerHTML && this.$refs.selectStreet.innerHTML && this.howntown) {
                let tempArr = document.querySelector('.selectCity').innerHTML.split(' ')
                let tempStr = tempArr[0] + '省' + tempArr[1] + '市' + tempArr[2]
                let street = this.$refs.selectStreet.innerHTML.trim()
        ...
        selectStreet() {
            if (document.querySelector('.selectCity').innerHTML === '选择省市') {
                Toast({
                    message: '请先选择省市区',
                    duration: 1000
                })
    ...
    mounted() {
        zbl_cityPickerIndex(document.querySelector('.selectCity'))
    },
